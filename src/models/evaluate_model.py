...
def evaluate_model(model_path: str, test_features_path: str, test_target_path: str, predictions_output_path: str, scores_output_path: str) -> None:
    """
    Loads a trained regression model, evaluates its performance on the test set,
    saves the predictions to a CSV file, and saves the evaluation metrics to a JSON file.

    Args:
        model_path: Path to the trained model .pkl file.
        test_features_path: Path to the scaled testing features CSV file (X_test_scaled.csv).
        test_target_path: Path to the testing target CSV file (y_test.csv).
        predictions_output_path: Path to save the predictions as a CSV file.
        scores_output_path: Path to save the evaluation metrics as a JSON file.
    """
    try:
